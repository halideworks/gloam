...
The site in `site/` is static and ships no runtime JavaScript beyond the small
inline theme and slider scripts on each page. There is no bundler and nothing
server-side, but there are now two generators that run by hand and commit their
output, both for the whitepaper:

```sh
npm ci                                    # KaTeX, build-time only
node scripts/build-whitepaper-math.mjs    # typeset the equations
python scripts/build-whitepaper-md.py     # regenerate the Markdown copy
```

Run them in that order after editing `site/whitepaper.html`, since the Markdown
is derived from the typeset page. Neither is wired into CI: the deploy workflow
publishes `site/` exactly as committed, so a forgotten run shows up as stale
output in the diff rather than as a broken deploy.
...
Nothing fingerprints the stylesheet, so its filename never changes on its own.
Each page links `style.css?v=<hash>`; the hash is the first 8 characters of the
file's MD5:
...
The whitepaper's equations are LaTeX, rendered to static HTML by KaTeX at build
time. The page carries no maths JavaScript: `scripts/build-whitepaper-math.mjs`
writes the rendered markup into `site/whitepaper.html` before it is committed.

The source is the `data-tex` attribute on each `.eq` and `.m` element, seeded
from `scripts/whitepaper-equations.json` for the display equations. Editing an
equation means editing that attribute (or the JSON) and re-running the script,
which replaces the rendered span from `data-tex` every time and is therefore
idempotent.

`site/assets/katex/` holds a trimmed copy of KaTeX's stylesheet and only the
seven font files the page actually pulls. That set was derived by loading the
page and reading `performance.getEntriesByType('resource')`; re-derive it the
same way if new notation starts reaching for a face that is not there, since a
missing font degrades quietly to a fallback glyph rather than erroring.

This is why `font-src` in the CSP carries `'self'` alongside Google Fonts.
