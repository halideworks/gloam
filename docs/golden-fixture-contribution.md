...
- **A colorimeter Gloam supports.** Anything ArgyllCMS `spotread` can drive. Note the exact
  model — it goes in the manifest and determines the uncertainty budget's instrument class.
...
2. Choose the monitor, display type, target, and preset. Use a **Thorough** or **Full**
   preset — the adaptive presets measure fewer patches, and a sparse recording pins less.
...
| `--panel` | Human-readable label shown in test output. Include the panel technology in parentheses — that is the whole point of your contribution. |
...
Your fixture is now a test case. It should pass immediately — you just generated the
...
- **Pull request** adding the fixture directory. Preferred — it runs through CI on the way
